...
# --- HELPER: DOWNLOAD BINARY DENGAN VALIDASI ---
# Mengunduh binary lalu memverifikasi bahwa hasil download benar-benar
# file ELF (bukan halaman error/HTML kosong dari GitHub), sebelum di-chmod +x.
# Ini tidak menggantikan checksum pinning (upstream tidak menerbitkan hash resmi
# untuk build ini), tapi mencegah instalasi biner kosong/corrupt/halaman-error.
function safe_download_binary() {
    local url="$1" dest="$2" name="$3"
    local tmp
    tmp=$(mktemp)
    if ! wget -q -O "$tmp" "$url"; then
        echo -e "${RED}GAGAL: download $name dari $url (koneksi/URL bermasalah).${NC}" >&2
        rm -f "$tmp"
        return 1
    fi
    if [[ ! -s "$tmp" ]]; then
        echo -e "${RED}GAGAL: file $name hasil download kosong.${NC}" >&2
        rm -f "$tmp"
        return 1
    fi
    if ! file "$tmp" | grep -qi "ELF"; then
        echo -e "${RED}GAGAL: file $name bukan binary ELF yang valid (kemungkinan halaman error). Instalasi dibatalkan untuk komponen ini.${NC}" >&2
        rm -f "$tmp"
        return 1
    fi
    mv "$tmp" "$dest"
...
# --- HELPER: VALIDASI USERNAME/PASSWORD ---
# Membatasi input hanya huruf, angka, titik, garis bawah, dan strip.
# Mencegah karakter '|' (merusak format file data), '/' (path traversal ke
# folder quota), spasi, dan karakter shell berbahaya lainnya.
function valid_input() {
    local val="$1"
    [[ "$val" =~ ^[a-zA-Z0-9._-]+$ ]]
}

# --- HELPER: UPDATE CONFIG JSON DENGAN AMAN ---
# Menjalankan filter jq ke file config, memastikan hasilnya JSON valid
# sebelum menimpa file asli. Mencegah config.json rusak/kosong akibat
# error jq di tengah jalan (yang membuat xray gagal restart tanpa pesan jelas).
function safe_jq_update() {
    local config_file="$1" filter="$2"; shift 2
    local tmp
    tmp=$(mktemp)
    if jq "$@" "$filter" "$config_file" > "$tmp" 2>/dev/null && jq empty "$tmp" 2>/dev/null; then
        mv "$tmp" "$config_file"
        chmod 644 "$config_file"
        return 0
    else
        echo -e "${RED}GAGAL update config (jq error) — perubahan dibatalkan, config tidak diubah.${NC}" >&2
        rm -f "$tmp"
        return 1
    fi
}
...
(
    ...
    apt-get install -y -q -o Dpkg::Options::="--force-confdef" -o Dpkg::Options::="--force-confold" curl socat jq openssl uuid-runtime net-tools vnstat wget gnupg1 bc iproute2 iptables iptables-persistent python3 python3-pip neofetch cron zip unzip stunnel4 bzip2 zlib1g-dev build-essential gcc make cmake fail2ban file
...
(
    ...
    if [[ "$dom_opt" == "1" ]]; then
        ...
        CF_RESP=$(curl -s -X POST "https://api.cloudflare.com/client/v4/zones/${CF_ZONE_ID}/dns_records" \
             ...
             --data '{"type":"A","name":"'${DOMAIN_VAL}'","content":"'${IP_VPS}'","ttl":120,"proxied":false}')
        if ! echo "$CF_RESP" | jq -e '.success == true' >/dev/null 2>&1; then
            echo "PERINGATAN: Gagal membuat DNS record di Cloudflare (cek CF_TOKEN/CF_ZONE_ID). Domain '$DOMAIN_VAL' mungkin belum resolve ke IP VPS." >&2
        fi
    else
        DOMAIN_VAL="$user_dom"
    fi
...
EOF
    ...
    # UDPGW (Badvpn)
    safe_download_binary "https://raw.githubusercontent.com/daybreakersx/prem/main/badvpn-udpgw64" "/usr/bin/badvpn-udpgw" "badvpn-udpgw"
...
(
    safe_download_binary "https://github.com/zahidbd2/udp-zivpn/releases/download/udp-zivpn_1.4.9/udp-zivpn-linux-amd64" "/usr/local/bin/zivpn" "zivpn"
...
function check_exists() {
    local user=$1
    if ! valid_input "$user"; then
        echo -e "${RED}Username '$user' tidak valid! Hanya boleh huruf, angka, titik, garis bawah, dan strip (tanpa spasi/simbol).${NC}"
        sleep 2
        return 1
    fi
...
function check_uuid() {
    local uuid=$1
    if ! valid_input "$uuid"; then
        echo -e "${RED}Password '$uuid' tidak valid! Hanya boleh huruf, angka, titik, garis bawah, dan strip (tanpa simbol '|').${NC}"
        sleep 2
        return 1
    fi
...
function bot_menu() {
    while true; do header_sub
        ...
        case $opt in
            1) read -p " Bot Token: " bt; read -p " Chat ID: " ci; echo "$bt" > /etc/tendo_bot/bot_token; echo "$ci" > /etc/tendo_bot/chat_id; chmod 600 /etc/tendo_bot/bot_token /etc/tendo_bot/chat_id 2>/dev/null; echo -e "${GREEN}Saved Successfully!${NC}"; sleep 1;;
            2) menu_login_notif ;;
            3) menu_backup_notif ;;
            x) return;;
        esac
...
function setup_client_bot_menu() {
    ...
    if [[ "$chg" == "y" || "$chg" == "Y" || -z "$c_tok" ]]; then
        ...
        chmod 600 /etc/tendo_bot/client_token /etc/tendo_bot/client_admin 2>/dev/null
    else
    ...
    while true; do header_sub
        ...
        case $opt in
            ...
            5) u="trial-$(tr -dc a-z0-9 </dev/urandom | head -c 5)"; echo -e " Username (Trial): ${GREEN}$u${NC}"; p="$u"; read -p " Duration (e.g., 10m, 1h): " dur;
               if [[ "$dur" == *m ]]; then add_str="+${dur%m} minutes"; elif [[ "$dur" == *h ]]; then add_str="+${dur%h} hours"; else echo -e "${RED}Format durasi salah (gunakan contoh: 10m atau 1h). Default 1 jam dipakai.${NC}"; sleep 2; add_str="+1 hours"; fi
            ...
            10) 
               ...
               if [[ -n "$link_res" ]]; then
                   ...
                   if [[ -f "/root/restore.zip" ]]; then
                       ...
                       # Permission dikunci (bukan 777): direktori 700, file token 600
                       # agar bot token tidak bisa dibaca/ditulis user lain di server.
                       chmod 700 /etc/tendo_bot/ 2>/dev/null
                       chmod 600 /etc/tendo_bot/*_token /etc/tendo_bot/chat_id /etc/tendo_bot/client_admin 2>/dev/null
                       systemctl daemon-reload

...
(
    ...
    # jadi log file cukup dimiliki root. Ini menghindari bentrok permission
    # dengan proses lain (bot Telegram, cron) yang semuanya juga jalan root.
...
(
    ...
    # FIX PERMANEN (bug: "xray mati tiap kali bot Telegram buat akun" /
    # "xray off setelah instalasi"):
    # 1) Installer resmi XTLS di atas menjalankan xray.service sebagai
    #    User=nobody DAN menimpa ulang ownership /var/log/xray ke nobody:nogroup
    #    (menimpa chown root:root yang sudah kita set di step Domain & SSL).
    #    Bot Telegram & semua script CLI di sini jalan sebagai root, jadi
    #    setiap kali menulis config.json (mktemp+mv), ada celah singkat file
    #    balik ke root & mode 600 sebelum chmod 644 sempat jalan -> kalau
    #    ketiban restart xray, xray gagal start "permission denied".
    # 2) Sebagian unit file xray.service dari installer resmi juga memakai
    #    directive sandboxing systemd (ProtectSystem/ReadWritePaths dll) yang
    #    bisa memblokir tulis ke /var/log/xray meski service dijalankan
    #    sebagai root, karena root pun tunduk pada mount-namespace systemd.
    # Solusinya: paksa xray jalan sebagai root DAN matikan sandboxing yang
    # bisa mengunci /var/log/xray & /usr/local/etc/xray, lalu re-chown log
    # (harus dilakukan SETELAH installer di atas, karena installer-lah yang
    # menimpa ownership-nya).
...
#!/bin/bash
export PATH="/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin"
# Pakai lock yang sama dengan xray-exp/xray-limit/xray-quota (cron), supaya
# akun yang dibuat lewat bot Telegram tidak bentrok/tertimpa dengan proses
# cron yang jalan tiap menit dan sama-sama menulis config.json.
[ "$XRAY_CFG_LOCKED" != "1" ] && exec env XRAY_CFG_LOCKED=1 flock /var/lock/xray-config.lock "$0" "$@"
